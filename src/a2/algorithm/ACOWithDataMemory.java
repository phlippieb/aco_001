...
import a2.ant.DataMemoryAnt;
import a2.ant.RememberedData;
import a2.data.Data;
...
public class ACOWithDataMemory {
    ...
    public ACOWithDataMemory (int xSize, int ySize, int numberOfAnts, int numberOfData, int dataSize, int dataRange, int antMemorySize) {

        //init grid and ant array
        this.grid = new Grid (xSize, ySize);
        this.ants = new DataMemoryAnt [numberOfAnts];
        ...
        for (int i = 0; i < numberOfAnts; i++) {
            ...
            DataMemoryAnt newAnt = new DataMemoryAnt (randomX, randomY, antMemorySize);
    ...
    public void iterate () {
        ...
        for (DataMemoryAnt ant : ants) {
            ...
            if (!ant.hasData() && grid.cellHasData(antX, antY)) {
                ...
                if (randomBoolean(probability)) {
                    //remember data and put down
                    ant.remember(antX, antY, ant.getData().clone());
            ...
            if (ant.hasData() && ant.hasMemory()) {
                if (randomBoolean (this.memoryBias)) {
                    // calculate best fit:
                    RememberedData bestData = calculateBestFit (ant.getData(), ant.getMemory(), this.grid);
                    // move towards best fit:
                    moveAntTowards(ant, bestData.x, bestData.y);
                } else {
                    //ant.clearMemory(); // only if working with densities?
                    moveAnt (ant, grid);
                }
            } else {
                moveAnt (ant, grid);
    ...
    protected void moveAnt (DataMemoryAnt ant, Grid grid) {
    ...
    void moveAntTowards (DataMemoryAnt ant, int x, int y) {
    ...
    private RememberedData calculateBestFit(Data data, RememberedData[] memories, Grid grid) {
        //Cell bestCell = null;
        RememberedData bestData = null;
        for (RememberedData memory : memories) {
            if (bestData == null) {
                bestData = memory.clone();
            } else {
                if (this.dataComparator.getDistanceBetween(data, memory.d) <
                        this.dataComparator.getDistanceBetween(data, bestData.d)) {
                    bestData = memory.clone();
                }
            }
        }
